...
- 2026-09-16: Fees undo — admin could record a payment but not remove it from the student's row. Added deletePaymentRow() (undo one month of a receipt), paymentRows in getFeeBoard, an "Undo" list inside the month modal, a "Receipts" button on fully-paid rows, and "Rs X · undo" under a paid month. Tested on a throwaway intake + student (recorded full batch, undid Month 2 from the month cell and Month 1 from Receipts, totals and history went back to 0) and deleted the test rows — real data unchanged (5 students, 7 payments, Rs 14,000).
